...
try {
    ...
    if ($event_type === 'historial') {
        // Obtener detalles del historial médico (solo del doctor actual)
        $medicalHistory = new MedicalHistory($db);
        $historial = $medicalHistory->obtenerPorId($event_id);
        
        if (!$historial) {
            echo '<div class="alert alert-danger">Registro de historial médico no encontrado o no tiene permisos para verlo.</div>';
            exit;
        }
        ...
        // Obtener detalles de la cita (solo del doctor actual)
        $appointment = new Appointment($db);
        $cita = $appointment->obtenerPorId($event_id);
        
        if (!$cita) {
            echo '<div class="alert alert-danger">Cita médica no encontrada o no tiene permisos para verla.</div>';
            exit;
        }
        ...
        // Obtener detalles de la imagen (solo del doctor actual)
        $medicalImage = new MedicalImage($db);
        $imagen = $medicalImage->obtenerPorId($event_id);
        
        if (!$imagen) {
            echo '<div class="alert alert-danger">Imagen médica no encontrada o no tiene permisos para verla.</div>';
            exit;
        }
        ...
        // Obtener detalles de la cotización (solo del doctor actual)
        $cotizacion = new Cotizacion($db);
        $cotizacion_data = $cotizacion->obtenerPorId($event_id);
        
        if (!$cotizacion_data) {
            echo '<div class="alert alert-danger">Cotización no encontrada o no tiene permisos para verla.</div>';
            exit;
        }
